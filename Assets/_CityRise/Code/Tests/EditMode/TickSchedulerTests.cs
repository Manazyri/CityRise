...
public sealed class TickSchedulerTests
{
    private sealed class CountingStep : ITickStep
    {
        ...
        public Result<Unit> Run(in TickContext context)
        {
            RunCount++;
    ...
    {
        var s = new TickScheduler { Speed = SpeedMultiplier.Fast };
        var step = new CountingStep("a");
        s.RegisterSim(step);
    ...
    {
        var s = new TickScheduler { Speed = SpeedMultiplier.Paused };
        var step = new CountingStep("a");
        s.RegisterSim(step);
    ...
    {
        var s = new TickScheduler { Speed = SpeedMultiplier.Faster };  // 3x = 60 ticks in 20 real seconds
        var budget = new CountingStep("budget");
        s.RegisterBudget(budget);

        s.Update(20d);  // 60 sim ticks at 3x
